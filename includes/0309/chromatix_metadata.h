...
#define METADATA_HEADER_VERSION 0x0003
 ...
 *    @tuning_size: array of meta data sizes
 *
 **/
typedef struct {
  uint32_t version;
  uint32_t tuning_size[PPROC_META_DATA_MAX_IDX];
} pproc_meta_header_t;
...
/** cpp_info_t
 *    @size:         size of cpp frame message
 *    @cpp_frame_msg: cpp frame message
 *
 **/
typedef struct {
  uint32_t   size;
  uint32_t   cpp_frame_msg[4096];
} cpp_info_t;
